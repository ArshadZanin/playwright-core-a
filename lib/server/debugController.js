...
var _ariaSnapshot2 = require("../utils/isomorphic/ariaSnapshot");
...
class DebugController extends _instrumentation.SdkObject {
  ...
  async highlight(params) {
    ...
    if (params.ariaTemplate) {
      parsedYaml = (0, _ariaSnapshot.parseYamlForAriaSnapshot)(params.ariaTemplate);
      (0, _ariaSnapshot2.parseYamlTemplate)(parsedYaml);
    }
    for (const recorder of await this._allRecorders()) {
      if (parsedYaml) recorder.setHighlightedAriaTemplate(parsedYaml);else if (params.selector) recorder.setHighlightedSelector(this._sdkLanguage, params.selector);
    }
  }
  async hideHighlight() {
