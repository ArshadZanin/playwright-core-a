...
class BrowserContextDispatcher extends _dispatcher.Dispatcher {
  constructor(parentScope, context) {
    ...
    /// RecordPlay(smartester-v1) STEP 7: Listen for the 'onAction' event and forward it to the client-side
  ...
  // RecordControl(smartester-v1) STEP 3.1: Implement the startRecord methods
  async startRecord() {
    this._context.startRecord();
  }
  // RecordControl(smartester-v1) STEP 3.2: Implement the stopRecord methods
  async stopRecord() {
    this._context.stopRecord();
  }
  // RecordControl(smartester-v1) STEP 3.3: Implement the set to locator mode methods
  async setToLocatorMode() {
    this._context.setToLocatorMode();
  }
  // RecordControl(smartester-v1) STEP 3.4: Implement the method to check if the context is in record mode
  async isRecord(params, metadata) {
    return {
      record: await this._context.isRecord()
    };
  }
  // RecordControl(smartester-v1) STEP 3.5: Implement the method to check if the context is in locator mode
  async isLocatorModeActive(params, metadata) {
    return {
      active: await this._context.isLocatorModeActive()
    };
  }
  // RecordControl(smartester-v1) STEP 3.6: Register a new method for getting the pause status
  async isPauseMode(params, metadata) {
    return {
      isPause: await this._context.isPauseMode()
    };
  }
  // RecordControl(smartester-v1) STEP 3.7: Register a new method for set to Standby mode
  async setToStandbyMode(params, metadata) {
    await this._context.setToStandbyMode();
  }
  _onDispose() {
