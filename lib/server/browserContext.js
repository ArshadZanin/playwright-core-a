...
/// RecordPlay(smartester-v1) STEP 6.2: Import the action module
...
class BrowserContext extends _instrumentation.SdkObject {
  constructor(browser, options, browserContextId) {
    ...
    // RecordControl(smartester-v1) STEP 5: Add a private field to store the RecorderApp instance
  ...
  async _initialize() {
    ...
    this._debugger.on(_debugger.Debugger.Events.PausedStateChanged, () => {
      if (this._debugger.isPaused()) {
        _recorder.Recorder.showInspectorNoReply(this, _recorderApp.RecorderApp.factory(this)).then(recorder => {
          // RecordControl(smartester-v1) STEP 6: store the Recorder instance
          if (recorder) {
  ...
  /// RecordPlay(smartester-v1) STEP 6.3: Emit the onAction event when any action occurred
  async emitOnAction(action) {
    if ((await this.isRecord()) === false) return;
    this.emit(BrowserContext.Events.OnAction, action);
  }

  // RecordControl(smartester-v1) STEP 4.1: Register a new method for starting the recording
  async startRecord() {
    if (!(await this.isPauseMode())) {
      console.log("Failed to startRecord: The page is not in pause mode");
      return;
    }
    if (this._recorder == undefined) {
      console.log("Failed to startRecord: The recorder is not initialized");
      return;
    }
    await this.attribution.playwright.debugController.setRecorderMode({
      mode: 'recording'
    });
  }
  // RecordControl(smartester-v1) STEP 4.2: Register a new method for stopping the recording
  async stopRecord() {
    if (!(await this.isPauseMode())) {
      console.log("Failed to stopRecord: The page is not in pause mode");
      return;
    }
    if (this._recorder == undefined) {
      console.log("Failed to stopRecord: The recorder is not initialized");
      return;
    }
    if (this._recorder.mode() !== 'recording') {
      console.log('Failed to stopRecord: The recorder is not in recording mode');
    }
    await this.attribution.playwright.debugController.setRecorderMode({
      mode: 'standby'
    });
  }
  // RecordControl(smartester-v1) STEP 4.3: Register a new method for setting to the locator mode
  async setToLocatorMode() {
    if (!(await this.isPauseMode())) {
      console.log("Failed to setToLocatorMode: The page is not in pause mode");
      return;
    }
    if (this._recorder == undefined) {
      console.log("Failed to setToLocatorMode: The recorder is not initialized");
      return;
    }
    await this.attribution.playwright.debugController.setRecorderMode({
      mode: 'inspecting'
  ...
  // RecordControl(smartester-v1) STEP 4.4: Register a new method for getting the locator mode status
  async isLocatorModeActive() {
    if (this._recorder == undefined || !(await this.isPauseMode())) {
      return false;
    }
    return this._recorder.mode() == 'inspecting';
  }
  // RecordControl(smartester-v1) STEP 4.5: Register a new method for getting the record status
  async isRecord() {
    if (this._recorder == undefined || !(await this.isPauseMode())) {
      return false;
    }
    return this._recorder.mode() == 'recording';
  }
  // RecordControl(smartester-v1) STEP 4.6: Register a new method for getting the pause status
  async isPauseMode() {
    return this._debugger.isPaused();
  }
  // RecordControl(smartester-v1) STEP 4.7: Register a new method for set to Standby mode
  async setToStandbyMode() {
    if (!(await this.isPauseMode())) {
      console.log("Failed to setToStandbyMode: The page is not in pause mode");
      return;
    }
    if (this._recorder == undefined) {
      console.log("Failed to setToStandbyMode: The recorder is not initialized");
      return;
    }
    await this.attribution.playwright.debugController.setRecorderMode({
      mode: 'standby'
...
BrowserContext.Events = {
  ...
  /// RecordPlay(smartester-v1) STEP 4: Register a new event for the action callback
  OnAction: 'onaction'
};
