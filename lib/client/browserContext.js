...
class BrowserContext extends _channelOwner.ChannelOwner {
  ...
  constructor(parent, type, guid, initializer) {
    ...
    /// RecordPlay(smartester-v1) STEP 8: Listen for the 'OnAction' event from the server and emit it to the user code
    this._channel.on('onaction', ({
      action
    }) => {
      if (!action) return;
      const dataObject = {
      ...
      this.emit(_events.Events.BrowserContext.OnAction, dataObject);
    });
  ...
  // RecordControl(smartester-v1) STEP 2.1: Add method for starting recording
  async startRecord() {
    await this._channel.startRecord();
  }
  // RecordControl(smartester-v1) STEP 2.2: Add method for stopping recording
  async stopRecord() {
    await this._channel.stopRecord();
  }
  // RecordControl(smartester-v1) STEP 2.3: Add method for set to locator mode
  async setToLocatorMode() {
    await this._channel.setToLocatorMode();
  }
  // RecordControl(smartester-v1) STEP 2.4: Add method to check if locator mode is active
  async isLocatorModeActive() {
    return (await this._channel.isLocatorModeActive()).active;
  }
  // RecordControl(smartester-v1) STEP 2.5: Add method to check if recording is active
  async isRecord() {
    return (await this._channel.isRecord()).record;
  }
  // RecordControl(smartester-v1) STEP 2.6: Register a new method for getting the pause status
  async isPauseMode() {
    return (await this._channel.isPauseMode()).isPause;
  }
  // RecordControl(smartester-v1) STEP 2.7: Register a new method for set to Standby mode
  async setToStandbyMode() {
    await this._channel.setToStandbyMode();
  }
  _disposeHarRouters() {
